...
void MouseMotion( int x, int y )
{
    ...
    {
        CameraRotate_y += 2.0 * (x - _mouse_buttonX)/(float)glutGet( GLUT_WINDOW_WIDTH );
        _mouse_buttonX = x;
        CameraRotate_x -= 2.0 * (_mouse_buttonY - y)/(float)glutGet( GLUT_WINDOW_HEIGHT );
        _mouse_buttonY = y;
        glutPostRedisplay();
        myCamera.Rotate(CameraRotate_x, CameraRotate_y);
    }
}
